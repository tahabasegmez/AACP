...
const makeSut = () => {
    ...
    return { store, auth: new AuthService_1.AuthService(store, 'test-secret-key', 3600) };
};
describe('password', () => {
    it('doğru şifreyi doğrular', async () => {
        const hash = await (0, password_1.hashPassword)('sifre12345');
        expect(await (0, password_1.verifyPassword)('sifre12345', hash)).toBe(true);
    });
    it('yanlış şifreyi reddeder', async () => {
        const hash = await (0, password_1.hashPassword)('sifre12345');
        expect(await (0, password_1.verifyPassword)('baskasifre', hash)).toBe(false);
    });
    it('bozuk özet biçiminde false döner', async () => {
        expect(await (0, password_1.verifyPassword)('sifre12345', 'bozuk')).toBe(false);
    });
    it('aynı şifre farklı özet üretir (salt)', async () => {
        const a = await (0, password_1.hashPassword)('sifre12345');
        const b = await (0, password_1.hashPassword)('sifre12345');
        expect(a).not.toBe(b);
    });
});
