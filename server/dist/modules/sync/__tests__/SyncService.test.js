...
const makeSut = () => {
    ...
    return { store, sync: new SyncService_1.SyncService(store) };
};
const record = (key, updatedAt, value = '{}', deleted = false) => ({
    key,
    value,
    updatedAt,
    deleted,
...
describe('SyncService', () => {
    ...
    it('geçersiz kayıtları reddeder', async () => {
        const { sync } = makeSut();
        await expect(sync.push('u1', 'progress', [{ key: '', updatedAt: 1 }])).rejects.toThrow(/key zorunlu/);
        await expect(sync.push('u1', 'progress', [{ key: 'a', updatedAt: 0 }])).rejects.toThrow(/updatedAt/);
    });
});
